...
- Latest OMI dB/PDM gain alignment is pushed and the pre-DevKit2-correction `0181e7f` `mobile`, `firmware`, and `tools` Actions succeeded. Flashing the exact DevKit2 `MIC_GAIN 64` artifact is pending because the PC currently sees only the Android phone over USB, not the mojizo COM port or `XIAO-SENSE` UF2 drive.
